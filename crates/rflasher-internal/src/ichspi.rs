...
use crate::ich_regs::*;
use crate::pci::{pci_read_config32, pci_write_config8, pci_read_config8};
...
impl Default for Opcodes {
    fn default() -> Self {
        ...
        Self {
            ...
            opcode: [
                Opcode { opcode: JEDEC_BYTE_PROGRAM, spi_type: SPI_OPCODE_TYPE_WRITE_WITH_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_READ, spi_type: SPI_OPCODE_TYPE_READ_WITH_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_SE, spi_type: SPI_OPCODE_TYPE_WRITE_WITH_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_RDSR, spi_type: SPI_OPCODE_TYPE_READ_NO_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_REMS, spi_type: SPI_OPCODE_TYPE_READ_WITH_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_WRSR, spi_type: SPI_OPCODE_TYPE_WRITE_NO_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_RDID, spi_type: SPI_OPCODE_TYPE_READ_NO_ADDRESS, atomic: 0 },
                Opcode { opcode: JEDEC_CE_C7, spi_type: SPI_OPCODE_TYPE_WRITE_NO_ADDRESS, atomic: 0 },
            ],
        }
    }
}
...
impl IchSpiController {
    ...
    fn get_spibar_address(chipset: &DetectedChipset) -> Result<u64, InternalError> {
        ...
        if gen.is_pch100_compatible() {
            ...
            Err(InternalError::NotSupported("Unsupported chipset generation"))
        }
    }
    ...
    fn init_ich9(&mut self) -> Result<(), InternalError> {
        ...
        if hsfs & HSFS_FDOPSS == 0 && self.desc_valid {
            log::info!("Flash Descriptor Override Strap-Pin is set. \
        ...
        if self.desc_valid && !self.generation.is_pch100_compatible() 
    ...
    fn handle_access_permissions(&mut self) -> Result<(), InternalError> {
        ...
        for i in 0..num_freg {
            ...
            // Check permissions
            let can_read = if i < max_regions { (bm_rap >> i) & 1 != 0 } else { true };
            let can_write = if i < max_regions { (bm_wap >> i) & 1 != 0 } else { true };
            let prot = AccessProtection::from_permissions(can_read, can_write);
            
            if prot != AccessProtection::None {
                restricted = true;
                log::info!("FREG{}: Region {:#010x}-{:#010x} is {:?}", 
                          i, base, limit, prot);
            } else {
                log::debug!("FREG{}: Region {:#010x}-{:#010x} is read-write", 
                           i, base, limit);
            }
        }
        
        if restricted {
            log::warn!("Not all flash regions are freely accessible. \
    ...
    fn handle_protected_ranges(&mut self) {
        let num_pr = if self.generation.is_pch100_compatible() { 6 } else { 5 };
    ...
    fn set_bbar(&mut self, min_addr: u32) {
        ...
        if self.bbar != (min_addr & BBAR_MASK) {
            log::warn!("Setting BBAR to {:#010x} failed! New value: {:#010x}",
                      min_addr, self.bbar);
        }
    }
    ...
    fn determine_mode(&mut self) -> Result<(), InternalError> {
        ...
        if effective_mode == SpiMode::Auto {
            ...
            if self.locked && self.missing_opcodes() {
                log::info!("Enabling hardware sequencing because some \
    ...
    fn find_opcode(&self, opcode: u8) -> Option<usize> {
        self.opcodes.as_ref()?.opcode.iter().position(|op| op.opcode == opcode)
    }
    
    /// Print HSFS register bits
    fn print_hsfs(&self, hsfs: u16) {
        log::debug!("HSFS: FDONE={} FCERR={} AEL={} SCIP={} FDV={} FLOCKDN={}",
                   (hsfs & HSFS_FDONE) != 0,
                   (hsfs & HSFS_FCERR) != 0,
                   (hsfs & HSFS_AEL) != 0,
                   (hsfs & HSFS_SCIP) != 0,
    ...
    fn print_opcodes(&self, opcodes: &Opcodes) {
        log::debug!("Preop: [{:#04x}, {:#04x}]", opcodes.preop[0], opcodes.preop[1]);
        for (i, op) in opcodes.opcode.iter().enumerate() {
    ...
    fn hwseq_set_addr(&self, addr: u32) {
        self.spibar.write32(ICH9_REG_FADDR, addr & self.hwseq.addr_mask);
    }
    ...
    pub fn hwseq_read(&self, addr: u32, buf: &mut [u8]) -> Result<(), InternalError> {
        ...
        while remaining > 0 {
            ...
            let block_len = remaining.min(HWSEQ_MAX_DATA).min(256 - (current_addr as usize & 0xFF));
            
            self.hwseq_set_addr(current_addr);
            
            // Clear status and set up read
            self.spibar.write16(ICH9_REG_HSFS, self.spibar.read16(ICH9_REG_HSFS));
    ...
    pub fn hwseq_erase(&self, addr: u32, len: u32) -> Result<(), InternalError> {
        ...
        let erase_size = if self.hwseq.only_4k { 4096 } else { 
            // TODO: Read actual erase size from BERASE bits
            4096 
        };
        
        if addr % erase_size != 0 || len % erase_size != 0 {
            return Err(InternalError::Io("Erase address/length not aligned to erase block size"));
        }
...
impl IchSpiController {
    pub fn new(_chipset: &DetectedChipset, _mode: SpiMode) -> Result<Self, InternalError> {
        Err(InternalError::NotSupported("Intel internal programmer only supported on Linux"))
    }
}
