...
pub fn enable_amd_spi100(
    ...
    impl PciConfigAccess for LinuxPci {
        fn read8(&self, bdf: Bdf, offset: u16) -> Result<u8, InternalError> {
            crate::pci::pci_read_config8(bdf.bus, bdf.device, bdf.function, offset as u8)
        }

        fn read16(&self, bdf: Bdf, offset: u16) -> Result<u16, InternalError> {
            crate::pci::pci_read_config16(bdf.bus, bdf.device, bdf.function, offset as u8)
        }

        fn read32(&self, bdf: Bdf, offset: u16) -> Result<u32, InternalError> {
            pci_read_config32(bdf.bus, bdf.device, bdf.function, offset as u8)
        }

        fn write8(&self, bdf: Bdf, offset: u16, value: u8) -> Result<(), InternalError> {
            crate::pci::pci_write_config8(bdf.bus, bdf.device, bdf.function, offset as u8, value)
        }

        fn write16(&self, bdf: Bdf, offset: u16, value: u16) -> Result<(), InternalError> {
            crate::pci::pci_write_config16(bdf.bus, bdf.device, bdf.function, offset as u8, value)
        }

        fn write32(&self, bdf: Bdf, offset: u16, value: u32) -> Result<(), InternalError> {
            crate::pci::pci_write_config32(bdf.bus, bdf.device, bdf.function, offset as u8, value)
        }
    }

    enable_amd_spi100_with_host(&LinuxPci, enable, Bdf::new(bus, device, 0), revision_id)
}
...
mod tests {
    ...
    fn test_enable_amd_spi100_with_host_extracts_bars() {
        ...
        assert_eq!(info.memory_addr, Some(0));
