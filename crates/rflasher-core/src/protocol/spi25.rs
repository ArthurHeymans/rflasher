...
pub fn read_status1<M: SpiMaster>(master: &mut M) -> Result<u8> {
    let mut buf = [0u8; 1];
...
pub fn read_status2<M: SpiMaster>(master: &mut M) -> Result<u8> {
    let mut buf = [0u8; 1];
...
pub fn read_status3<M: SpiMaster>(master: &mut M) -> Result<u8> {
    let mut buf = [0u8; 1];
...
pub fn write_enable<M: SpiMaster>(master: &mut M) -> Result<()> {
    let mut cmd = SpiCommand::simple(opcodes::WREN);
    master.execute(&mut cmd)
}

/// Send the Write Disable command
pub fn write_disable<M: SpiMaster>(master: &mut M) -> Result<()> {
...
pub fn wait_ready<M: SpiMaster>(master: &mut M, timeout_us: u32) -> Result<()> {
...
pub fn write_status1<M: SpiMaster>(master: &mut M, value: u8) -> Result<()> {
    write_enable(master)?;
    let data = [value];
...
pub fn write_status12<M: SpiMaster>(master: &mut M, sr1: u8, sr2: u8) -> Result<()> {
    write_enable(master)?;
    let data = [sr1, sr2];
    let mut cmd = SpiCommand::write_reg(opcodes::WRSR, &data);
    master.execute(&mut cmd)?;
...
pub fn read_3b<M: SpiMaster>(master: &mut M, addr: u32, buf: &mut [u8]) -> Result<()> {
...
pub fn read_4b<M: SpiMaster>(master: &mut M, addr: u32, buf: &mut [u8]) -> Result<()> {
    let max_len = master.max_read_len();
    let mut offset = 0;
...
pub fn program_page_3b<M: SpiMaster>(
    master: &mut M,
    addr: u32,
    data: &[u8],
    timeout_us: u32,
...
pub fn program_page_4b<M: SpiMaster>(
    master: &mut M,
    addr: u32,
    data: &[u8],
    timeout_us: u32,
...
pub fn erase_block<M: SpiMaster>(
    master: &mut M,
    opcode: u8,
    addr: u32,
    use_4byte: bool,
...
pub fn chip_erase<M: SpiMaster>(master: &mut M, timeout_us: u32) -> Result<()> {
    write_enable(master)?;
...
pub fn enter_4byte_mode<M: SpiMaster>(master: &mut M) -> Result<()> {
    let mut cmd = SpiCommand::simple(opcodes::EN4B);
    master.execute(&mut cmd)
}

/// Exit 4-byte address mode
pub fn exit_4byte_mode<M: SpiMaster>(master: &mut M) -> Result<()> {
    let mut cmd = SpiCommand::simple(opcodes::EX4B);
    master.execute(&mut cmd)
}

/// Send software reset sequence
pub fn software_reset<M: SpiMaster>(master: &mut M) -> Result<()> {
...
pub fn read_sfdp<M: SpiMaster>(master: &mut M, addr: u32, buf: &mut [u8]) -> Result<()> {
...
pub fn check_wel<M: SpiMaster>(master: &mut M) -> Result<bool> {
    let status = read_status1(master)?;
    Ok(status & opcodes::SR1_WEL != 0)
}

/// Check if a write or erase operation is in progress
pub fn is_busy<M: SpiMaster>(master: &mut M) -> Result<bool> {
    let status = read_status1(master)?;
