...
pub async fn prepare_io<M: SpiMaster + ?Sized>(
    ...
    if wants_quad {
        ...
        if method == QuadEnableMethod::None {
            // Without a declared QE method the QE bit cannot be established,
            // so IO2/IO3 stay WP#/HOLD# and a quad read would return garbage.
            log::warn!(
                "{} is quad-capable but declares no QE method; disabling quad read modes for this session",
                ctx.chip.name
            );
    ...
    let mut read_op = protocol::select_read_op(
        master_features,
        caps,
        dc,
        use_4byte,
    ...
    .ok_or(crate::error::Error::ChipNotSupported)?;

    // ------ 4. Select and establish the address strategy ------
    let (read_addressing, entered_4ba) = if use_4byte {
    ...
    if entered_4ba {
        protocol::enter_4byte_mode_with_features(master, chip_features).await?;
    }
...
mod tests {
    ...
    fn prepare_skips_qe_when_already_set() {
        futures_lite::future::block_on(async {
            ...
            prepare_io(&mut ctx, &mut master).await.unwrap();
