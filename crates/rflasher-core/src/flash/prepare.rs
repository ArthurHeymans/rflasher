//! Flash preparation and teardown
//!
//! Mirrors flashprog's `spi_prepare_io` / `spi_finish_io` (`spi25_prepare.c`).
//!
//! Before any flash operation, we need to:
//!
//! 1. Set 4-byte addressing mode if the chip requires it.
//! 2. Set the Quad-Enable bit if the programmer + chip both support any
//!    quad-IO read, so that IO2/IO3 become functional.
//! 3. Optionally enter QPI (4-4-4) mode if both sides support it.
//! 4. Pick the fastest read operation available and cache it on the context.
//!
//! On finish, undo the steps that have side-effects on the chip.

use crate::chip::Features;
use crate::error::Result;
use crate::flash::context::{AddressMode, FlashContext};
use crate::programmer::{SpiFeatures, SpiMaster};
use crate::protocol::{
    self, ChipReadCapabilities, CommandAddressing, DummyCycleOverrides, QuadEnableMethod, SpiReadOp,
};

/// Cached per-session state assembled by `prepare_io`.
///
/// Stored alongside the `FlashContext` (not mutated at read-time) so the
/// flash device can dispatch to the right read op and can undo side-effects
/// in `finish_io` (exit QPI, restore the QE bit; retain 4-byte addressing).
#[derive(Debug, Clone, Copy)]
pub struct PreparedState {
    /// Chosen read operation (opcode + mode + dummy cycles + address width)
    pub read_op: SpiReadOp,
    /// Addressing strategy used with `read_op`.
    pub read_addressing: CommandAddressing,
    /// True if QPI mode is active for this session.
    pub in_qpi_mode: bool,
    /// Opcode to use when exiting QPI (0x00 if we didn't enter QPI).
    pub qpi_exit_opcode: u8,
    /// True if `prepare_io` entered 4-byte addressing via EN4B.
    ///
    /// Operations use this to skip redundant EN4B/EX4B bracketing. `finish_io`
    /// deliberately leaves the mode active, matching flashprog.
    pub entered_4ba: bool,
    /// True if a volatile QE write may have taken effect for this session.
    ///
    /// `finish_io` clears it again, mirroring flashprog's
    /// `volatile_qe_enabled` restore. False when QE was already set on entry
    /// (pre-existing state is left untouched) or no write was attempted.
    pub volatile_qe_enabled: bool,
    /// QE method used for the volatile set/clear above (`None` if unused).
    pub qe_method: QuadEnableMethod,
}

impl Default for PreparedState {
    fn default() -> Self {
        Self {
            read_op: SpiReadOp::sio_read(),
            read_addressing: CommandAddressing::ThreeByte,
            in_qpi_mode: false,
            qpi_exit_opcode: 0x00,
            entered_4ba: false,
            volatile_qe_enabled: false,
            qe_method: QuadEnableMethod::None,
        }
    }
}

impl PreparedState {
    /// Create a session state for a chip when no programmer is available
    /// (e.g. WASM probe-only contexts). Falls back to slow single-I/O 0x03.
    pub fn default_for(ctx: &FlashContext) -> Self {
        let mut state = Self::default();
        if ctx.address_mode == AddressMode::FourByte {
            if ctx.chip.features.contains(Features::FOUR_BYTE_READ) {
                state.read_op = SpiReadOp::sio_read_4b();
                state.read_addressing = CommandAddressing::FourByte;
            } else if ctx.chip.features.supports_4ba_mode_switch() {
                state.read_op.address_width = crate::spi::AddressWidth::FourByte;
                state.read_addressing = CommandAddressing::FourByte;
            } else if ctx.chip.features.supports_extended_address_register() {
                state.read_addressing =
                    CommandAddressing::ExtendedAddressRegister(ctx.chip.features);
            }
        }
        state
    }
}

fn chip_to_caps(features: Features, in_qpi_mode: bool) -> ChipReadCapabilities {
    ChipReadCapabilities {
        fast_read: features.contains(Features::FAST_READ),
        dout: features.contains(Features::FAST_READ_DOUT),
        dio: features.contains(Features::FAST_READ_DIO),
        qout: features.contains(Features::FAST_READ_QOUT),
        qio: features.contains(Features::FAST_READ_QIO),
        qpi_fast_read: features.intersects(Features::ANY_QPI) || in_qpi_mode,
        qpi4b: features.contains(Features::FAST_READ_QPI4B),
        native_4ba_read: features.contains(Features::FOUR_BYTE_READ),
        native_4ba_fast_read: features.contains(Features::FOUR_BYTE_FAST_READ),
        native_4ba_dout: features.contains(Features::FOUR_BYTE_DUAL_OUT_READ),
        native_4ba_dio: features.contains(Features::FOUR_BYTE_DUAL_IO_READ),
        native_4ba_qout: features.contains(Features::FOUR_BYTE_QUAD_OUT_READ),
        native_4ba_qio: features.contains(Features::FOUR_BYTE_QUAD_IO_READ),
        in_qpi_mode,
    }
}

fn chip_qe_method(ctx: &FlashContext) -> QuadEnableMethod {
    use crate::chip::QeMethod;
    match ctx.chip.qe_method {
        QeMethod::None => QuadEnableMethod::None,
        QeMethod::Sr2Bit1WriteSr => QuadEnableMethod::Sr2Bit1WriteSr,
        QeMethod::Sr2Bit1WriteSr2 => QuadEnableMethod::Sr2Bit1WriteSr2,
        QeMethod::Sr1Bit6 => QuadEnableMethod::Sr1Bit6,
        QeMethod::Sr2Bit7 => QuadEnableMethod::Sr2Bit7,
    }
}

/// Prepare a flash chip for multi-IO operation.
///
/// Should be called after `probe`-ing and constructing the `FlashContext`,
/// but before issuing any read/write/erase. Returns a `PreparedState` that
/// the flash-device layer should carry for the session's lifetime.
///
/// Degrade-vs-fail contract:
/// - QE bit unset or unwritable -> quad disabled, dual/single continue.
/// - No accelerated read op selectable -> single-I/O 0x03 fallback, except
///   on 4-byte chips (no 3-byte fallback can reach them) which fail to open.
/// - 4-byte addressing required but unsupported by the master/chip pair ->
///   `Err` (fatal: the chip is not addressable through this programmer).
///   Callers surface this from `open`; it is not silently downgraded because
///   a wrong-width address would corrupt data.
pub async fn prepare_io<M: SpiMaster + ?Sized>(
    ctx: &mut FlashContext,
    master: &mut M,
) -> Result<PreparedState> {
    let mut state = PreparedState::default_for(ctx);
    match prepare_io_in_place(ctx, master, &mut state).await {
        Ok(()) => Ok(state),
        Err(error) => {
            finish_io(&state, master).await?;
            Err(error)
        }
    }
}

// Adapters retain this state even on failure, so teardown can retry restoration.
pub(crate) async fn prepare_io_in_place<M: SpiMaster + ?Sized>(
    ctx: &mut FlashContext,
    master: &mut M,
    state: &mut PreparedState,
) -> Result<()> {
    restore_temporary_qe(state, master).await?;
    let master_features = master.features();
    let chip_features = ctx.chip.features;

    // ------ 1. Quad Enable (volatile, mirrors flashprog spi_prepare_quad_io) ------
    //
    // The QE bit is set with an EWSR-prefixed (volatile) write so it does
    // not survive power cycles, confirmed with a re-read, and restored
    // (cleared) by finish_io. A write the chip ignores or rejects therefore
    // degrades to non-quad instead of silently issuing garbage quad reads.
    let mut effective_chip_features = chip_features;
    let mut volatile_qe_enabled = false;
    let wants_quad = chip_features.intersects(Features::ANY_QUAD)
        && master_features
            .intersects(SpiFeatures::QUAD_IN | SpiFeatures::QUAD_IO | SpiFeatures::QPI);
    let qe_method = chip_qe_method(ctx);
    if wants_quad {
        if qe_method == QuadEnableMethod::None {
            // No QE register on this chip (e.g. factory-enabled parts):
            // flashprog leaves quad alone in this case, so do we. Nothing
            // to establish and nothing to restore.
            log::debug!(
                "{} declares quad modes without a QE register; assuming QE is factory-set",
                ctx.chip.name
            );
        } else {
            match protocol::is_quad_enabled(master, qe_method).await {
                Ok(true) => {
                    log::debug!("QE bit already set for {}", ctx.chip.name);
                }
                Ok(false) => {
                    // flashprog only attempts the volatile set on chips
                    // with volatile SR-write support; otherwise an unset QE
                    // means quad stays off for this session.
                    if !chip_features.contains(Features::WRSR_EWSR) {
                        log::info!(
                            "QE bit unset and {} has no volatile status-register writes; disabling quad read modes for this session",
                            ctx.chip.name
                        );
                        effective_chip_features &= !Features::ANY_QUAD;
                    } else {
                        log::info!("Setting volatile QE bit for {}", ctx.chip.name);
                        // Own restoration before submission: transport/poll errors do not
                        // prove the register write failed to reach the chip.
                        volatile_qe_enabled = true;
                        state.volatile_qe_enabled = true;
                        state.qe_method = qe_method;
                        let enabled = protocol::enable_quad_mode_volatile(master, qe_method)
                            .await
                            .is_ok()
                            && protocol::is_quad_enabled(master, qe_method)
                                .await
                                .unwrap_or(false);
                        if !enabled {
                            log::warn!(
                                "QE bit did not confirm after volatile write; disabling quad read modes for this session"
                            );
                            effective_chip_features &= !Features::ANY_QUAD;
                        }
                    }
                }
                Err(e) => {
                    log::warn!(
                        "Could not read QE bit ({e:?}); disabling quad read modes for this session"
                    );
                    effective_chip_features &= !Features::ANY_QUAD;
                }
            }
        }
    }

    // ------ 2. QPI entry ------
    //
    // QPI (4-4-4) mode rewires ALL SPI commands — including status reads,
    // WREN, erase, and program — into 4-wire framing. The rest of our
    // flash operation pipeline (erase/write/WP/status polling paths in
    // `protocol::spi25` and the programmer `execute()` path) currently
    // issues single-IO commands unconditionally. Entering QPI here and
    // leaving it enabled across those operations would hang or corrupt.
    //
    // For now we deliberately skip QPI entry. The fine-grained 1-2-2 /
    // 1-1-4 / 1-4-4 paths already give the bulk of the speedup. QPI
    // support will require a broader refactor that brackets non-read ops
    // with `exit_qpi_with` / `enter_qpi_with`, mirroring flashprog's
    // `spi_prepare_io` / `spi_finish_io`.
    let in_qpi_mode = false;
    let qpi_exit_opcode = 0x00u8;
    if effective_chip_features.intersects(Features::ANY_QPI)
        && master_features.contains(SpiFeatures::QPI)
    {
        log::debug!(
            "QPI supported by both master and {} but not enabled (QPI-aware \
             non-read command paths not yet implemented)",
            ctx.chip.name,
        );
    }

    // ------ 3. Pick read op ------
    let mut caps = chip_to_caps(effective_chip_features, in_qpi_mode);
    let dc = DummyCycleOverrides {
        dc_112: ctx.chip.dummy_cycles_112,
        dc_122: ctx.chip.dummy_cycles_122,
        dc_114: ctx.chip.dummy_cycles_114,
        dc_144: ctx.chip.dummy_cycles_144,
        dc_qpi: ctx.chip.dummy_cycles_qpi,
    };
    use crate::spi::IoMode;
    caps.dout &= master
        .supports_read_dummy_cycles(IoMode::DualOut, if dc.dc_112 == 0 { 8 } else { dc.dc_112 });
    caps.dio &= master
        .supports_read_dummy_cycles(IoMode::DualIo, if dc.dc_122 == 0 { 4 } else { dc.dc_122 });
    caps.qout &= master
        .supports_read_dummy_cycles(IoMode::QuadOut, if dc.dc_114 == 0 { 8 } else { dc.dc_114 });
    caps.qio &= master
        .supports_read_dummy_cycles(IoMode::QuadIo, if dc.dc_144 == 0 { 6 } else { dc.dc_144 });
    let use_4byte = ctx.address_mode == AddressMode::FourByte;
    let allow_compatibility_4ba = !use_4byte
        || super::operations::compatible_4byte_addressing(chip_features, master_features).is_ok();
    let mut read_op = match protocol::select_read_op(
        master_features,
        caps,
        dc,
        use_4byte,
        allow_compatibility_4ba,
        |opcode| master.probe_opcode(opcode),
    ) {
        Some(op) => op,
        // No accelerated op is selectable (e.g. master/chip share nothing
        // beyond plain reads): degrade to single-I/O 0x03 rather than fail
        // the open — but only where 3-byte addressing can reach the whole
        // chip. A 4-byte chip with no selectable op is genuinely
        // unaddressable, so the open fails instead of aliasing into the
        // low 16 MiB.
        None if use_4byte => {
            // Same restore as the addressing/EN4B failure paths below: a
            // volatile QE bit set above must not leak through a failed open.
            // NOTE: the in-QPI-mode None arm of select_read_op (exit QPI and
            // retry) is currently unreachable — QPI entry stays disabled and
            // in_qpi_mode is always false here. Implement the exit-retry
            // when QPI entry lands instead of extending this fallback.
            return Err(crate::error::Error::ChipNotSupported);
        }
        None => {
            log::warn!(
                "No multi-IO read op selectable for {}; falling back to single-I/O READ",
                ctx.chip.name
            );
            SpiReadOp::sio_read()
        }
    };

    // ------ 4. Select and establish the address strategy ------
    // The caller rolls back owned QE on failure and adapters retain any
    // unconfirmed restoration obligation for a subsequent teardown retry.
    let address_result = if use_4byte {
        super::operations::addressing_for_4byte_operation(
            read_op.native_4ba,
            chip_features,
            master_features,
        )
    } else {
        Ok((CommandAddressing::ThreeByte, false))
    };
    let (read_addressing, entered_4ba) = address_result?;
    read_op.address_width = match read_addressing {
        CommandAddressing::FourByte => crate::spi::AddressWidth::FourByte,
        CommandAddressing::ThreeByte | CommandAddressing::ExtendedAddressRegister(_) => {
            crate::spi::AddressWidth::ThreeByte
        }
    };
    if entered_4ba
        && !state.entered_4ba
        && let Err(e) = protocol::enter_4byte_mode_with_features(master, chip_features).await
    {
        return Err(e);
    }

    log::debug!(
        "Selected read op for {}: opcode=0x{:02X} io_mode={:?} dummy={} native_4ba={}",
        ctx.chip.name,
        read_op.opcode,
        read_op.io_mode,
        read_op.dummy_cycles,
        read_op.native_4ba,
    );

    *state = PreparedState {
        read_op,
        read_addressing,
        in_qpi_mode,
        qpi_exit_opcode,
        entered_4ba: entered_4ba || state.entered_4ba,
        volatile_qe_enabled,
        qe_method,
    };
    Ok(())
}

/// Undo side-effects from `prepare_io`.
///
/// Safe to call even if `prepare_io` partially failed — each step checks
/// whether the corresponding entry was taken.
///
/// A volatile QE bit set by `prepare_io` is cleared again here, mirroring
/// flashprog's `spi_finish_io` ("Trying to restore volatile quad-enable").
/// Pre-existing (sticky) QE state is never touched: only a bit this session
/// set is restored.
pub async fn finish_io<M: SpiMaster + ?Sized>(state: &PreparedState, master: &mut M) -> Result<()> {
    if state.in_qpi_mode
        && let Err(e) = protocol::exit_qpi_with(master, state.qpi_exit_opcode).await
    {
        log::warn!("exit_qpi_with failed: {e:?}");
    }
    let mut remaining = *state;
    restore_temporary_qe(&mut remaining, master).await?;
    // Match flashprog: leave compatibility 4BA mode active on release.
    Ok(())
}

/// Clear only session-owned QE and require readback before a persistent WP RMW.
/// Keep ownership on every error so later teardown/preparation can retry.
pub(crate) async fn restore_temporary_qe<M: SpiMaster + ?Sized>(
    state: &mut PreparedState,
    master: &mut M,
) -> Result<()> {
    if state.volatile_qe_enabled {
        protocol::disable_quad_mode_volatile(master, state.qe_method).await?;
        if protocol::is_quad_enabled(master, state.qe_method).await? {
            return Err(crate::error::Error::ProgrammerError);
        }
        state.volatile_qe_enabled = false;
    }
    Ok(())
}

// ============================================================================
// Tests
// ============================================================================

#[cfg(all(test, feature = "alloc"))]
mod tests {
    use super::*;
    use crate::chip::{EraseBlock, FlashChip, QeMethod, WriteGranularity};
    use crate::error::Result as CoreResult;
    use crate::programmer::SpiMaster;
    use crate::spi::{SpiCommand, opcodes};
    use alloc::vec;
    use alloc::vec::Vec;

    /// Minimal mock programmer that records every executed opcode and can
    /// have canned responses for register reads. Used to verify the prepare
    /// flow issues the right commands in the right order.
    struct MockMaster {
        features: SpiFeatures,
        sr1: u8,
        sr2: u8,
        /// When true, status-register writes are silently ignored
        /// (models a write-protected or rejected QE write).
        ignore_sr_writes: bool,
        executed: Vec<u8>,
    }

    impl MockMaster {
        fn new(features: SpiFeatures) -> Self {
            Self {
                features,
                sr1: 0,
                sr2: 0,
                ignore_sr_writes: false,
                executed: Vec::new(),
            }
        }
    }

    impl SpiMaster for MockMaster {
        fn features(&self) -> SpiFeatures {
            self.features
        }
        fn max_read_len(&self) -> usize {
            usize::MAX
        }
        fn max_write_len(&self) -> usize {
            usize::MAX
        }
        async fn execute(&mut self, cmd: &mut SpiCommand<'_>) -> CoreResult<()> {
            self.executed.push(cmd.opcode);
            match cmd.opcode {
                // RDSR1: return sr1
                opcodes::RDSR => {
                    if !cmd.read_buf.is_empty() {
                        cmd.read_buf[0] = self.sr1;
                    }
                }
                // RDSR2: return sr2
                opcodes::RDSR2 => {
                    if !cmd.read_buf.is_empty() {
                        cmd.read_buf[0] = self.sr2;
                    }
                }
                // WRSR: SR1 followed optionally by SR2
                opcodes::WRSR => {
                    if !cmd.write_data.is_empty() {
                        self.sr1 = cmd.write_data[0];
                    }
                    if cmd.write_data.len() >= 2 {
                        self.sr2 = cmd.write_data[1];
                    }
                }
                // WRSR2
                opcodes::WRSR2 if !cmd.write_data.is_empty() && !self.ignore_sr_writes => {
                    self.sr2 = cmd.write_data[0];
                }
                _ => {}
            }
            Ok(())
        }
        async fn delay_us(&mut self, _us: u32) {}
    }

    fn test_chip_quad() -> FlashChip {
        FlashChip {
            vendor: "Test".into(),
            name: "TQ".into(),
            jedec_manufacturer: 0xEF,
            jedec_device: 0x4018,
            total_size: 16 * 1024 * 1024,
            page_size: 256,
            features: crate::chip::Features::FAST_READ
                | crate::chip::Features::FAST_READ_DOUT
                | crate::chip::Features::FAST_READ_DIO
                | crate::chip::Features::FAST_READ_QOUT
                | crate::chip::Features::FAST_READ_QIO
                | crate::chip::Features::WRSR_EWSR,
            voltage_min_mv: 2700,
            voltage_max_mv: 3600,
            write_granularity: WriteGranularity::Page,
            erase_blocks: vec![EraseBlock::new(0xC7, 16 * 1024 * 1024)],
            tested: Default::default(),
            qe_method: QeMethod::Sr2Bit1WriteSr2,
            dummy_cycles_112: 0,
            dummy_cycles_122: 0,
            dummy_cycles_114: 0,
            dummy_cycles_144: 0,
            dummy_cycles_qpi: 0,
        }
    }

    #[test]
    fn prepare_enables_qe_when_not_set() {
        futures_lite::future::block_on(async {
            let chip = test_chip_quad();
            let mut ctx = FlashContext::new(chip);
            let mut master = MockMaster::new(
                SpiFeatures::QUAD_IN | SpiFeatures::QUAD_IO | SpiFeatures::FOUR_BYTE_ADDR,
            );
            // sr2 starts at 0, so QE bit is off -> should trigger a write.
            let state = prepare_io(&mut ctx, &mut master).await.unwrap();
            // With QIO flag set and master supports quad, we expect QuadIo read op.
            assert_eq!(state.read_op.io_mode, crate::spi::IoMode::QuadIo);
            // Check that RDSR2 was read and WRSR2 was issued, volatile
            // (EWSR-prefixed: the EWSR opcode precedes the WRSR2).
            assert!(master.executed.contains(&opcodes::RDSR2));
            assert!(master.executed.contains(&opcodes::WRSR2));
            assert!(master.executed.contains(&opcodes::EWSR));
            assert!(state.volatile_qe_enabled);
            // QE bit should now be set and confirmed.
            assert_eq!(master.sr2 & opcodes::SR2_QE, opcodes::SR2_QE);
            // finish() restores (clears) the volatile QE bit.
            finish_io(&state, &mut master).await.unwrap();
            assert_eq!(master.sr2 & opcodes::SR2_QE, 0);
        })
    }

    #[test]
    fn prepare_skips_qe_when_already_set() {
        futures_lite::future::block_on(async {
            let chip = test_chip_quad();
            let mut ctx = FlashContext::new(chip);
            let mut master = MockMaster::new(
                SpiFeatures::QUAD_IN | SpiFeatures::QUAD_IO | SpiFeatures::FOUR_BYTE_ADDR,
            );
            // Pre-set QE bit.
            master.sr2 = opcodes::SR2_QE;
            let state = prepare_io(&mut ctx, &mut master).await.unwrap();
            // RDSR2 read, but WRSR2 not issued.
            assert!(master.executed.contains(&opcodes::RDSR2));
            assert!(!master.executed.contains(&opcodes::WRSR2));
            assert!(!state.volatile_qe_enabled);
            // finish() must leave the pre-existing sticky QE bit alone.
            finish_io(&state, &mut master).await.unwrap();
            assert_eq!(master.sr2 & opcodes::SR2_QE, opcodes::SR2_QE);
        })
    }

    #[test]
    fn prepare_disables_quad_when_write_rejected() {
        futures_lite::future::block_on(async {
            let chip = test_chip_quad();
            let mut ctx = FlashContext::new(chip);
            let mut master = MockMaster::new(
                SpiFeatures::QUAD_IN | SpiFeatures::QUAD_IO | SpiFeatures::FOUR_BYTE_ADDR,
            );
            // The chip ignores the QE write (e.g. write-protected SR).
            master.ignore_sr_writes = true;
            let state = prepare_io(&mut ctx, &mut master).await.unwrap();
            // No confirmation -> degrade, but retain the restoration obligation.
            assert!(state.volatile_qe_enabled);
            assert_eq!(state.read_op.io_mode, crate::spi::IoMode::Single);
        })
    }

    #[test]
    fn prepare_disables_quad_without_volatile_sr_support() {
        futures_lite::future::block_on(async {
            let mut chip = test_chip_quad();
            // No WRSR_EWSR: flashprog parity is to leave QE alone and
            // disable quad rather than program a sticky bit.
            chip.features -= crate::chip::Features::WRSR_EWSR;
            let mut ctx = FlashContext::new(chip);
            let mut master = MockMaster::new(
                SpiFeatures::QUAD_IN | SpiFeatures::QUAD_IO | SpiFeatures::FOUR_BYTE_ADDR,
            );
            let state = prepare_io(&mut ctx, &mut master).await.unwrap();
            assert!(!master.executed.contains(&opcodes::WRSR2));
            assert!(!state.volatile_qe_enabled);
            assert_eq!(state.read_op.io_mode, crate::spi::IoMode::Single);
        })
    }

    #[test]
    fn prepare_falls_back_to_single_without_quad_master() {
        futures_lite::future::block_on(async {
            let chip = test_chip_quad();
            let mut ctx = FlashContext::new(chip);
            let mut master = MockMaster::new(SpiFeatures::FOUR_BYTE_ADDR);
            let state = prepare_io(&mut ctx, &mut master).await.unwrap();
            assert_eq!(state.read_op.io_mode, crate::spi::IoMode::Single);
            // No QE interaction without quad master.
            assert!(!master.executed.contains(&opcodes::RDSR2));
        })
    }
}
