...
def load_flashprog() -> dict[tuple[str, str], dict]:
    ...
    for i, (vendor, name, start) in enumerate(headers):
        ...
        qe_method = None
        # flashprog QE initializers may carry a trailing access member,
        # e.g. `.qe = {STATUS2, 1, RW}` — accept (and ignore) it. STATUS2
        # bit 1 stays unmapped (see QE_WRITE_MAP) since its write semantics
        # (combined WRSR vs dedicated 0x31) can't be told from this field.
        qm = re.search(
            r"\.qe\s*=\s*\{\s*STATUS(\d)\s*,\s*(\d)(?:\s*,\s*(?:RO|RW))?\s*\}", body
        )
        if qm:
